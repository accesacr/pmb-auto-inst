...
apt-get update
apt-get install -y git-core lockfile-progs rubygems curl dnsutils lsb-release ruby ruby-dev autoconf autogen libtool
...
#REPOSITORY_URL=git://github.com/datauy/$SITE.git
REPOSITORY_URL=git://github.com/datauy/pormibarrio.git
...
EOF
    ...
    then
        ...
        cat > /etc/apt/sources.list.d/mysociety-extra.list <<EOF
# Debian mirror to use, including contrib and non-free:
#deb http://the.earth.li/debian/ jessie main contrib non-free
#deb-src http://the.earth.li/debian/ jessie main contrib non-free
...
#deb-src http://backports.debian.org/debian-backports jessie-backports main contrib non-free
EOF
...
add_website_to_nginx() {
    ...
    #sed -i "s,fixmystreet,pormibarrio," $NGINX_SITE_FILENAME # Se agregó esta linea por el cambio de nombre a pormibarrio
    if [ $DEFAULT_SERVER = true ]
    then
...
install_website_packages() {
    ...
    #sed -i '/postgresql-server-dev/d' PACKAGES_FILE # Se agregó
    #sed -i 's/postgresql-8.4/postgresql-9.4/' $PACKAGES_FILE # Se agregó
...
echo "development_install $DEVELOPMENT_INSTALL"


mv $REPOSITORY/bin/site-specific-install.sh $REPOSITORY/bin/site-specific-install.sh.bak 
cp /home/perl/site-specific-install.sh $REPOSITORY/bin/site-specific-install.sh

mv $REPOSITORY/bin/install-as-user $REPOSITORY/bin/install-as-user.bak
cp /home/perl/install-as-user $REPOSITORY/bin/install-as-user

echo "Succesfully changed 'site-specific-install' file for custom file."
